...
var bluefinBH = window.bluefinBH || (function (document, $) {
    ...
    that.ajaxDialog = function (url, options)
    {
        if(options == null)
        {
            options = {backdrop :'static',keyboard :true,closeButton: true};
        }
        ...
        dlg.find('.dialog-body').load(url, function() {
            ...
            if (contentWidth > bodyWidth) {
                box.css('width', contentWidth+50+ 'px').css('margin-left', (-contentWidth/2-25) + 'px');
            ...
            if (modalOverflow)
            {
                box.css('top', 5).css('margin-top', 0).css('max-height', maxHeight - 30);
            }
            else
            {
    ...
    that.updateDialogSize = function(dialogBody)
    {

        var box = dialogBody.parent(),
            bodyWidth = dialogBody.width(),
            contentWidth = dialogBody.find('fieldset').width() || dialogBody.children().first().width();

        var modalOverflow = $(window).height() < box.height();
        box.toggleClass('page-overflow', modalOverflow)
            .toggleClass('modal-overflow', modalOverflow)
            .css('top', '50%')
            .css('margin-top', modalOverflow ? 0 : -box.height()/2);


        contentWidth += parseInt(dialogBody.css('padding-left')) + parseInt(dialogBody.css('padding-right'));
        ...
        dialogBody.css('max-height', '80%');
        dialogBody.on('click', 'button.btn-cancel', function(e) {
            box.modal("hide");
        });
...
var wbtAPI = window.wbtAPI || (function (document, $) {
    ...
    that.call = function (service, resourceID, params, success_function, fail_function) {

        if (arguments.length == 2) {
            var type = typeof resourceID;
            if (type == 'function') {
                success_function = resourceID;
                resourceID = null;
            }
            else if (type == 'object') {
                params = resourceID;
...
function showTips( $id ){ $("#" + $id).popover('show'); }
function hideTips( $id ){ $("#" + $id).popover('hide'); }
