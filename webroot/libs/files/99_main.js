 ...
 *************************/
	...
	_mapValue		: null,									// 鍦板浘鎵撳紑鏃讹紝瀛樺偍鏈€杩戞墦寮€鐨勪竴涓湴鍥�
	_mapIndex		: null,									// 寮€鍚湴鍥剧殑鍧愭爣浣嶇疆

	_audioNode		: $('.u-audio'),						// 澹伴煶妯″潡
	_audio			: null,									// 澹伴煶瀵硅薄
	_audio_val		: true,									// 澹伴煶鏄惁寮€鍚帶鍒�
 ...
 */
 	haddle_envent_fn : function(){
		...
		car2._on('success',function(){
			...
			// 鍒ゆ柇鏄惁鏄渶鍚庝竴椤碉紝璁╄交APP鍏宠仈椤甸潰闅愯棌
			...
			setTimeout(function(){
				...
				// 鍒ゆ柇鏄惁婊戝姩鏈€鍚庝竴椤碉紝骞惰杞籄PP浠嬬粛鍏宠仈椤甸潰璐ゆ窇
 ...
 */
	...
	mapAddEventHandler	 : function(obj,eventType,fn,option){
	    ...
	    if(!car2._isOwnEmpty(option)){
	        fnHandler = function(e){
	            fn.call(this, option);  //缁ф壙鐩戝惉鍑芥暟,骞朵紶鍏ュ弬鏁颁互鍒濆鍖�;
	        }
	    }
	    obj.each(function(){
	...
	mapShow : function(option){
		...
		setTimeout(function(){
			...
			if(car2._map.find('div').length>=1){
				...
				setTimeout(function(){
					//濡傛灉寮€鍚湴鍥剧殑浣嶇疆涓嶄竴鏍峰垯锛屽垱寤烘柊鐨勫湴鍥�
	...
	addMap	: function (detal,latitude,longitude,fnOpen,fnClose){
		...
		//鍒涘缓鍦板浘
		car2._map.ylmap({
			/*鍙傛暟浼犻€掞紝榛樿涓哄ぉ瀹夐棬鍧愭爣*/
			//闇€瑕佹墽琛岀殑鍑芥暟锛堝洖璋冿級
			detal		: detal,		//鍦板潃鍊�
			latitude	: latitude,		//绾害
			longitude	: longitude,	//缁忓害
			fnOpen		: fnOpen,		//鍥炶皟鍑芥暟锛屽湴鍥惧紑鍚墠
			fnClose		: fnClose		//鍥炶皟鍑芥暟锛屽湴鍥惧叧闂悗
		});	
	},
 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','files/loading_large.gif');
			}else{
				// 鎶婂師鏉ョ殑鍥剧墖棰勫厛淇濆瓨涓嬫潵
				...
				self.css({
					'background-image'	: 'url(files/loading_large.gif)',
	...
	regFunction	: function(inputName){
		...
		switch (inputName) {
			case 'name':
				...
				break;
			case 'sex':
				empty_tip = '鎯虫兂锛岃鎬庝箞绉板懠鎮ㄥ憿锛�';
				reg_tip = '鎯虫兂锛岃鎬庝箞绉板懠鎮ㄥ憿锛�';
				break;
			case 'tel':
				reg = /^1[0-9][0-9]\d{8}$/;
			...
			case 'email':
				...
				empty_tip = '閮�21涓栫邯浜嗭紝搴旇鏈変釜鐢靛瓙閭鍚э紒';
				reg_tip = '閭鏍煎紡鏈夐棶棰樺摝锛�';
				break;
			case 'company':
				reg = /^[\u4e00-\u9fa5|a-z|A-Z|\s|\d]{1,20}$/;
				empty_tip = '濉釜鍏徃鍚э紒';
				reg_tip = '杩欎釜鍏徃澶鎬簡锛�';
				break;
			case 'job':
				reg = /^[\u4e00-\u9fa5|a-z|A-Z|\s]{1,20}$/;
				empty_tip = '璇锋偍濉釜鑱屼綅';
				reg_tip = '杩欎釜鑱屼綅澶鎬簡锛�';
				break;
			case 'date':
				empty_tip = '缁欎釜鏃ユ湡鍚э紒';
				reg_tip = '';
