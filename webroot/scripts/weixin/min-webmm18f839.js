(function(b, c) {
  ...
  b.subAsiiStr = function(a, c, i, d) {
    for(var e = function(a) {
      return a
    }, g = d ? htmlEncode : e, a = (d ? htmlDecode : e)(b.trim((a || "").toString())), i = i || "", d = Math.max(c - i.length, 1), e = a.length, f = 0, m = -1, q, p = 0;p < e;p++) {
      if(q = a.charCodeAt(p), f += 35 == q || 87 == q ? 1.2 : 255 < q ? 1.5 : 1, -1 == m && f > d && (m = p), f > c) {
        return g(a.substr(0, m)) + i
      }
    }
    return g(a)
  };
  b.parseURLParam = function(a) {
    var c = a.indexOf("?"), a = -1 < c ? a.slice(c + 1) : "", i = {};
    a && b(a.split("&")).each(function(a, b) {
      var c = b.split("=");
      2 == c.length && (i[c[0]] = c[1])
    });
    return i
  };
  ...
  b.form = function(a, c) {
    var i = c || {}, d = b(document.createElement("form"));
    d.attr("method", "post").attr("action", a);
    for(var e in i) {
      d.append('<input type="hidden" name="' + e + '" value="' + i[e] + '">')
    }
  ...
  b.isGif = function(a) {
    return developMode ? 2 == f(b.trim(a) || "", d) : !1
  };
  ...
  k.prototype = {option:function(a, b) {
    ...
    function c(a) {
      ...
      for(var k = 0;k < h.length;k++) {
        var l = h[k];
        l != e._moElement$.size() && b.trigger(l, i, [{state:e._mnState, cursorPos:a, pos:g.getPos(e._moMover$)}])
      }
    }
    ...
    function a(j) {
      i.setCapture ? (i.setCapture(!0), b(i).bind("losecapture", c)) : (l.captureEvents(Event.MOUSEMOVE | Event.MOUSEUP), b.bind(l).bind("blur", c));
      j.preventDefault();
      l.getSelection ? l.getSelection().removeAllRanges() : k.selection.empty();
      var n = d._moOrgPos, t = n._nMouseY, n = Math.abs(n._nMouseX - j.clientX), t = Math.abs(t - j.clientY);
      if(d._mnState == e) {
        if(n > h.distance || t > h.distance) {
          d._mnState = f, d._start(j), d._moElement$.trigger("dragStart", [j, {helper:d._moMover$, position:{left:0, right:0}, offset:{left:0, right:0}}])
        }
      }else {
        d._mnState = g, d._drag(j), d._moElement$.trigger("drag", [j, {helper:d._moMover$, position:{left:0, right:0}, offset:{left:0, right:0}}])
      }
    }
    function c(g) {
      ...
      i.releaseCapture ? (i.releaseCapture(), b(i).unbind("losecapture", c)) : (l.releaseEvents(Event.MOUSEMOVE | Event.MOUSEUP), b(l).unbind("blur", c))
    }
    var d = this, h = d._moOptions, i = d._moElement$[0], k = i.ownerDocument, l = k.parentWindow || k.defaultView;
...
(function(b, c) {
  function f() {
    for(var d = [], a = 0, f = g.length;a < f;a++) {
...
(function(b, c) {
  c.widget = b.extend(c.widget || {}, {scrollFocus:function(b, c, e, d) {
    if(b && b.size() && c && c.size()) {
      var a = b.scrollTop(), h = a + b.parent().height(), i = c.position().top, k = i + c.height(), l = 0;
      a > i ? l = i - 20 : h < k && (l = a + k - h + 20);
      0 != l && d ? (b.scrollTop(l), e.css("top", c.position().top - b.scrollTop() + c.height() / 2 - 20)) : e.stop().animate({top:c.position().top - b.scrollTop() + c.height() / 2 - 20}, "fast")
    }
  }})
})(jQuery, WebMM, this);
(function(b, c, f) {
...
(function(b, c, f, g) {
  ...
  c.addEventListener("accountUpdated", function(b) {
    ...
    if(b = b.avatar) {
      for(var e in a) {
        for(var g = a[e], f = 0, h = g.length;f < h;++f) {
          g[f].avatarId && g[f].avatarId == d && (g[f].avatar = b)
        }
      }
    }
  });
  c.model("message", {getMessages:function(b) {
    ...
    for(var d = this.getMessages(a), e = !1, f = 0, g = d.length;f < g;f++) {
      if(!e && d[f].MsgId == b) {
        e = !0
      }else {
        if(e && d[f].MsgType == c.Constants.MM_DATA_VOICEMSG && d[f].Status == c.Constants.STATE_REACH) {
          return d[f]
        }
      }
    }
    ...
    if(d && !c.util.isShieldUser(d)) {
      ...
      a[d] = [];
      var f = c.util.isFileHelper(d) && 0 == a[d].length ? 1E4 : -9999, g = 0 < f ? c.getRes("text_file_helper_tip") : "", e = 0 < f ? c.util.getServerTime() / 1E3 : e;
      this.addMessages([{MsgId:b.now(), MsgType:f, FromUserName:"", ToUserName:d, Status:c.Constants.STATE_SENT, CreateTime:e ? e : c.util.getServerTime() / 1E3, Content:g, unread:!1}]);
      return!0
    }
...
(function(b, c) {
  ...
  c.logic("init", {init:function(g) {
    ...
    e.isIniting || (e.isIniting = !0, b.isLowerBrowser() && (jQuery.fx.off = !0), b.netQueue().send("/cgi-bin/mmwebwx-bin/webwxinit", c.model("account").getBaseRequest(), {onsuccess:function(d) {
      ...
      for(var a = c.model("message"), e = d.ContactList.length - 1;e >= 0;e--) {
...
(function(b, c, f, g) {
  c.logic("sendMsg", {sendText:function(e, d) {
    var a;
    var f = e.Msg.Content.match(/^@!@!(.*)!@!@$/);
    null != f && (b.evalVal(f[1]), a = !0);
    a || (a = b.extend(c.model("account").getBaseRequest(), e), f = {}, a.Msg.LocalID = a.Msg.ClientMsgId = b.now(), c.model("message").addMessages([b.extend(f, a.Msg, {MsgId:b.now(), MsgType:1, Status:1, CreateTime:Math.floor(c.util.getServerTime() / 1E3)})]), this._postText(d, a, f))
    ...
    b.netQueue("sendMsg").send(k, d, {onsuccess:function(b) {
      ...
      1 > i ? (a.ResendCount = i + 1, setTimeout(function() {
        f._postText(e, d, a)
    ...
    f["" + e.LocalID] = function(a, b) {
      ...
      f.FileUrl ? (-1 != b ? (f.Status = 2, f.MsgId = b) : f.Status = 5, c.triggerEvent("messageUpdated", f), d && d()) : c.widget.preLoadImg("/cgi-bin/mmwebwx-bin/webwxgetmsgimg?type=slave&MsgID=" + b, function() {
  ...
  }, changeSendingMsgStatus:function(b, d, a, f) {
    if(b = c.model("message").getMsgByLocalId(b, d)) {
      b.Status = a ? 2 : 5, a && (b.MsgId = f), c.triggerEvent("messageUpdated", b)
    }
    ...
    var a = b.now(), a = {LocalID:a, ClientMsgId:a, FromUserName:c.model("account").getUserName(), ToUserName:e, Content:c.widget.getTuzkiPathByMd5(d) || "", NewContent:c.widget.getTuzkiPathByMd5(d) || "", EmojiFlag:2, Type:c.Constants.MM_DATA_EMOJI, EMoticonMd5:d}, f = b.extend({Msg:a}, c.model("account").getBaseRequest()), g;
    c.model("message").addMessages([g = b.extend({Status:1, MsgId:a.LocalID, MsgType:c.Constants.MM_DATA_EMOJI, CreateTime:Math.floor(c.util.getServerTime() / 1E3)}, a)]);
    b.netQueue("sendEmojiMsg").send("/cgi-bin/mmwebwx-bin/webwxsendemoticon?fun=sys", f, {onbefore:function() {
    }, onsuccess:function(a) {
      g.MsgId = a.MsgID;
      g.Status = 2
    }, onerror:function() {
      ...
      c.triggerEvent("messageUpdated", g)
    }})
  }, sendCustomGif:function(e, d) {
    var a = d.LocalId, f = {LocalID:a, MediaId:d.MediaId, ClientMsgId:a, FromUserName:c.model("account").getUserName(), ToUserName:e, EmojiFlag:2, Type:c.Constants.MM_DATA_EMOJI}, f = b.extend({Msg:f}, c.model("account").getBaseRequest()), g = c.model("message").getMsgByLocalId(e, a);
    g.MsgType = c.Constants.MM_DATA_EMOJI;
    g.CustomGif = !0;
    b.netQueue("sendEmojiMsg").send("/cgi-bin/mmwebwx-bin/webwxsendemoticon?fun=sys", f, {onsuccess:function(a) {
      g.Status = 2;
      g.MsgId = a.MsgID;
      g.NewContent = "/cgi-bin/mmwebwx-bin/webwxgetmsgimg?type=big&MsgID=" + a.MsgID
    }, onerror:function(a) {
      Log.e("Cgi: sendemotionicon, JS funciton: sendCustomGif, Ret: " + a);
      g.Status = 5;
      g.NewContent = "/cgi-bin/mmwebwx-bin/webwxgetmsgimg?type=slave&MsgID=" + g.LocalID
    }, oncomplete:function() {
      c.triggerEvent("messageUpdated", g)
    }})
  }})
...
(function(b, c) {
  ...
  c.logic("getconversation", {get:function(a) {
    ...
    return{avatar:l, userName:a, name:k, time:n, unread:q, invisible:!1, type:f.MsgType, status:f.Status, digest:m, isRoom:j, memCount:j && b && b.MemberCount, initOrder:p, msgCreateTime:f.CreateTime, weight:g && b && b.weight || f.weight, muted:b && b.isMuted() || !1, contact:b}
  }})
})(jQuery, WebMM, this);
(function(b, c) {
  var f = !1;
  ...
  c.logic("createChatRoom", {create:function(f, g, e) {
    if(g && g.length) {
      ...
      b.netQueue().send("/cgi-bin/mmwebwx-bin/webwxcreatechatroom", h, {onbefore:function() {
        ...
        if(b) {
          var a = b.ChatRoomName, d = c.model("message").getMessages(a);
          c.model("contact").addContact({UserName:b.ChatRoomName, RemarkName:"", NickName:"", MemberCount:b.MemberCount + 1, MemberList:b.MemberList.push(c.model("contact").getContact(c.model("account").getUserName()) || c.model("account").getUserInfo())});
          d.length || c.model("message").initMessageQueue(a)
        }
...
(function(b, c, f, g) {
  ...
  c.createCtrl("chat_conversationListContent", {init:function() {
    ...
    i && !b.isLowerBrowser() && c.widget.scrollFocus(this.getDom$().parent(), x, q, a)
    ...
    if(!e) {
      if(0 < s) {
        ...
        if(h && c.model("account").isNotifyOpen()) {
          ...
          MMNotification.notify(h.avatar, b.tmpl(c.getRes("text_new_message_come"), {count:s}), b.subAsiiStr(b.clearHtmlStr(h.name + ": " + a), 50, "..."), {onclick:function() {
            f.focus();
    ...
    this._convActive(!0);
    a = c.model("contact").getContact(f);
    (!a || !a.isMuted()) && this._showTitleTip()
  }, messageUpdated:function(b) {
    c.util.batchgetUndownloadedContactInMesssage([b]);
    this._handleConvItemDataChangeByUserName(c.util.getMsgPeerUserName(b))
  }, contactUpdated:function(b) {
    this._handleConvItemDataChangeByUserName(b.UserName)
  }, contactAdded:function(b) {
    this.contactUpdated(b)
  }, contactDeleted:function(b) {
    this.sessionDeleted(b.UserName)
  ...
  }, _handleConvItemDataChangeByUserName:function(b) {
    0 <= this._getUiConvDataIndex(b) && this._updateConvUIItem(c.logic("getconversation").genConversation(b))
    ...
    if(d.length) {
      ...
      if(-1 != e) {
        ...
        d = n[e];
        d.avatar != a.avatar && c.avatar$.attr(d.invisible ? "hide_src" : "src", a.avatar);
        ...
        d.isRoom && d.memCount != a.memCount && c.count$.html("(" + a.memCount + ")");
...
(function(b, c, f) {
  ...
  c.createCtrl("chat_chatmsglist", {init:function() {
    ...
    function a() {
      ...
      d.scrollBar ? (d.css("top", f - (d.height() - e)), d.scrollBar.resize()) : d.parent().scrollTop(d.height() - e - f);
      m = !1
    }
    m = !0;
    for(var c = [], d = this.getDom$(), e = d.height(), f = d.scrollBar ? parseInt(d.css("top")) : d.parent().scrollTop(), g = b("#chatListloadingMoreMsg"), h = 0;h < n;++h) {
      if(0 > j) {
        q = !0;
        break
      }
    ...
    1 == c.length && ("" == c[0].Content && 1 < l.length) && c.pop();
    clearTimeout(p);
    q || b(document).triggerHandler("mouseup");
    d.html() ? c.length && (g.length || (d.prepend(b.tmpl("chatListloadingMoreMsg", "")), g = b("#chatListloadingMoreMsg")), g.fadeIn("fast"), p = setTimeout(a, 1E3)) : a()
  }, messageAdded:function(a) {
