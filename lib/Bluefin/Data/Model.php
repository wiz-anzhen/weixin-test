...
abstract class Model
{
    ...
    public static function fetchOneRow($selected, $condition = null, array $grouping = null, array $ranking = null, $top = 1, array &$outputColumns = null)
    {
        ...
        {
            if (in_array(Database::KW_SQL_CONDITION_LATEST, $condition, true))
            {
    ...
    public static function addRoleCondition($action, array &$condition)
    {
        ...
        foreach ($acl['roles'] as $auth => $roles)
        {
            ...
            if (in_array('*creator*', $roles, true))
            {
    ...
    {
        if (!in_array($name, $this->_metadata->getFieldNames(), true))
        {
            return;
        }
    ...
    public function load($condition)
    {
        if (!is_array($condition))
        {
            ...
            log_error("condition is not array. [table:{$this->metadata()->getModelName()}] .trace:", $trace);
    ...
    /*
     * 插入数据时，检查是否有unique key 重复的数据。
     * 成功插入返回 true； 有unique key重复的数据返回 false
     */
    public function insertInRestraintOfUniqueKey()
    {
        try
        {
            $this->insert();
        }
        catch(\Bluefin\Exception\DatabaseException $e)
        {
            if($e->getCode() == \Bluefin\Data\Db\PDO::ER_DUP_ENTRY)
            {
    ...
    // 暂时先用此方式， 后续考虑使用 on duplicate update 语法。
    ...
    public function insert($updateIfDuplicate = false)
    {
        ...
        $hasBeforeInsertTrigger = in_array('BEFORE-INSERT', $triggers, true);
        $hasAfterInsertTrigger = in_array('AFTER-INSERT', $triggers, true);
    ...
    /*
    * 更新数据时，检查是否有unique key 重复的数据。
    * 成功更新返回 true； 有unique key重复的数据返回 false
    */
    public function updateInRestraintOfUniqueKey()
    {
        try
        {
            $this->update();
    ...
    public function update($condition = null)
    {
        ...
        if (!empty($data))
        {
            ...
            $hasBeforeUpdateTrigger = in_array('BEFORE-UPDATE', $triggers, true);
            $hasAfterUpdateTrigger = in_array('AFTER-UPDATE', $triggers, true);
    ...
    public function delete($condition = null)
    {
        ...
        $hasBeforeDeleteTrigger = in_array('BEFORE-DELETE', $triggers, true);
        $hasAfterDeleteTrigger = in_array('AFTER-DELETE', $triggers, true);
    ...
    {
        if (!strict_in_array($columnName, $this->metadata()->getFieldNames()))
        {
