    ...
    protected function _query($sql, array $params = null)
    {
        ...
        try
        {
            ...
            if (false === $flag)
            {
                ...
                throw new \Bluefin\Exception\DatabaseException("SQL exception: {$eInfo[2]}".". sql:$sql", $errno);
            }
        ...
        catch (\PDOException $e)
        {
            $eInfo = $pdoStmt->errorInfo();
            $errno = $eInfo[1];
            throw new \Bluefin\Exception\DatabaseException($e->getMessage() .". sql:$sql",$errno , $e);
        }
    }
