...
class Field
{
    ...
    private function _processField($fieldTypeWithModifiers)
    {
        ...
        {
            if (strict_in_array($fieldType, Type::getBuiltinTypes()))
            {
    ...
    {
        //规范配置文件中的值的类型
        if (strict_in_array($this->_fieldType, [Type::TYPE_FLOAT, Type::TYPE_MONEY]))
        {
            if (array_key_exists(Type::ATTR_MIN, $this->_filters))
            {
        ...
        if (strict_in_array($this->_fieldType, array(Type::TYPE_DATE, Type::TYPE_TIME, Type::TYPE_DATE_TIME, Type::TYPE_TIMESTAMP)))
        {
            // TODO:
            return;
        }
