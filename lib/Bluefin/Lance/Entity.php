...
class Entity
{
        ...
        switch ($this->_status)
        {
            ...
            case Convention::ENTITY_STATUS_FINAL_CHECK:
                ...
                if ($this->_entityType == Convention::ENTITY_TYPE_RELATION)
                {
                    ...
                    if (isset($refEntity2))
                    {
                        $refEntity1->setM2NRelation($this->getCodeName(), $refInfo1[2], $fieldName1, $fieldName2, strict_in_array($fieldName1, $this->_relationKey), $refEntityName2, $refInfo2[2]);
                        $refEntity2->setM2NRelation($this->getCodeName(), $refInfo2[2], $fieldName2, $fieldName1, strict_in_array($fieldName2, $this->_relationKey), $refEntityName1, $refInfo1[2]);
                    }
                    else
                    {
                        $refEntity1->setM2NRelation($this->getCodeName(), $refInfo1[2], $refInfo1[0], strict_in_array($fieldName1, $this->_relationKey));
                    }
                }
                        ...
                        if (array_key_exists(Convention::KEYWORD_ENTITY_FSM_TRANSITIONS, $context))
                        {
                            ...
                            foreach ($transitions as $actionName => &$conditions)
                            {
                                ...
                                if (!$this->_hasActionApi &&
                                    ...
                                    strict_in_array('service', $conditions[Convention::KEYWORD_ENTITY_FSM_CALLER]))
                                {
    ...
    {
        if (strict_in_array($baseEntity->getEntityType(), array(Convention::ENTITY_TYPE_ENUM, Convention::ENTITY_TYPE_FST)))
        {
    ...
    private function _integrate(Entity $baseEntity, array $modifiers = null)
    {
        ...
        foreach ($rawFields as $fieldName => $fieldType)
        {
            ...
            if (strict_in_array($nameParts[0], $exclude))
            {
                continue;
            }
    ...
    private function _normalizeRoles(array &$conditions)
    {
        if (array_key_exists(Convention::KEYWORD_ENTITY_FSM_ALLOWED_ROLES, $conditions))
        {
            ...
            foreach ($roles as $auth => $authRoles)
            {
                ...
                if (strict_in_array(Convention::KEYWORD_ENTITY_FSM_ROLE_OWNER_TOKEN, $authRoles))
                {
                ...
                if (strict_in_array(Convention::KEYWORD_ENTITY_FSM_ROLE_CREATOR_TOKEN, $authRoles))
                {
