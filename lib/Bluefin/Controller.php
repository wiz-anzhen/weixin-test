...
abstract class Controller
{
    ...
    {
        $ok = $this->_view->render();
        //log_debug('jwlkrjlwqjf' . $ok);
    ...
    {
        if (!isset($this->_requestSource) && $useThisIfNoSource)
        {
            $this->_requestSource = $this->_request->getFullRequestUri();
        }

        if (isset($this->_requestSource))
        {
            $queryParams[Convention::KEYWORD_REQUEST_FROM] = b64_encode($this->_requestSource);
        }
