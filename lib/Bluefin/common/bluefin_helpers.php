 ...
 * @param array $fragmentParams
  * @return string
 */
function _U($relativeUrl = null, array $queryParams = null, array $fragmentParams = null)
{
    /**
     * @var \Bluefin\Gateway $gateway
     */
    $gateway = App::getInstance()->gateway();
    return $gateway->url($relativeUrl, $queryParams, $fragmentParams);
}
 ...
 * @return mixed
 */
function _QUERY($dbName, $sql, array $params = null)
{
    return App::getInstance()->db($dbName)->getAdapter()->fetchAll($sql, $params);
}
...
function log_debug_with_call_stack($msg)
{
    $trace = debug_backtrace();
    $depth = 0;
    $fun = '';
    if (isset($trace[$depth + 1]))
    {
        $fun = $trace[$depth + 1]['function'];
    }
    $file = basename($trace[$depth + 0]['file']);
    $line = $trace[$depth + 0]['line'];
    $funLineInfo =  "[file:$file][line:$line][fun:$fun]";


    $textTrace = '';
    unset($trace[0]); // 去掉本函数的调用记录

    foreach($trace as $row)
    {
        $textTrace .=$row['file'].'[line:'.$row['line'].'][fun:'.$row['function'] ."]\n";
    }

    App::getInstance()->log()->debug($funLineInfo . $msg . "\n" . $textTrace);
}

function log_error_with_call_stack($msg)
{
    $trace = debug_backtrace();
    $depth = 0;
    $fun = '';
    ...
    foreach($trace as $row)
    {
        $htmlTrace .=$row['file'].'[line:'.$row['line'].'][fun:'.$row['function'] .']<br>';
        $textTrace .=$row['file'].'[line:'.$row['line'].'][fun:'.$row['function'] ."]\n";
    }

    App::getInstance()->log()->error($funLineInfo . $msg . '<br><br>'. $htmlTrace  . "\n" . $textTrace);
}
...
function log_info($msg, $arr = null)
{
    ...
    App::getInstance()->log()->info($info . $msg . $str_arr);
}
