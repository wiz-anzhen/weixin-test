...
class DbAuth implements AuthInterface
{
    ...
    public function __construct($authName, array $config)
    {
        ...
        $this->_captchaClass = $config['captchaClass'];
    ...
    public function authenticate(array $authInput)
    {
        ...
        $session = App::getInstance()->session();
        $failedTimesKey = make_dot_name('auth', $this->_name, 'failedTimes');
        $session->increase($failedTimesKey);
        ...
        if ($this->_needCaptcha)
        {
            ...
            if (!$captcha->validateCaptchaInput($authInput))
            {
                return AuthHelper::FAILURE_VERIFY_CODE_INVALID;
            }
        }
        ...
        $this->setIdentity([ $this->_uidColumn => $model->__get($this->_uidColumn) ]);
        $session->remove($failedTimesKey);
