...
class Form extends Container
{
    ...
    const FIELD_UPLOAD_IMG = 'uploadImg';
    const FIELD_CHECKBOX_COUNT_PER_LINE = 'checkbox_count_per_line';
    ...
    const COM_CHECK_GROUP = 'checkGroup';
    ...
    const COM_IMAGE_UPLOAD = 'imageUpload';
    const COM_RICH_TEXT = 'richText';
    const COM_MOBILE_AUTH = 'mobileAuth';
    const COM_CUSTOM = 'custom';
    const COM_TIME_RANGE = 'timeRange';
    const COM_PCA = 'pca';
    ...
    public $buttons;
    // form 顶部和底部同时显示按钮
    public $showButtonsAtHeaderAndFooter = false;
    public $metadata;
    public $message;
    ...
    public static function fromModelMetadata(ModelMetadata $mainTableMeta, array $fields = null, $data /* or pkValue */ = null, array $attributes = null)
    {
        ...
        {//编辑模式
            $form->editMode = true;
            $form->addComponent(self::_buildFormComponent([self::FIELD_NAME => $mainTableMeta->getPrimaryKey(), self::FIELD_TAG => self::COM_HIDDEN, self::FIELD_VALUE => $pkValue]));
        ...
        foreach ($fields as $fieldName => $componentOption)
        {
            ...
            if (isset($fieldOptions))
            {
                ...
                else if($componentOption[self::FIELD_TAG] == self::COM_INPUT)
                {
    ...
    {
        // 如果未指定样式，统一使用 input-xlarge, 这样的效果是对齐的
        isset($componentOption[self::FIELD_CLASS]) || ($componentOption[self::FIELD_CLASS] = 'input-xlarge');
...
*/
    ...
    protected static function _buildFormComponent(array $componentOption)
    {
        ...
        switch ($tag)
        {
            case self::COM_INPUT:
                ...
                /*
                    if (array_try_get($componentOption, self::FIELD_REQUIRED, false))
                    {
...
*/
            ...
            case self::COM_CHECK_BOX:
               return new CheckBox($componentOption);

            case self::COM_TIME:
                $collection = array_try_get($componentOption, self::FIELD_DATA, [], true);
                return new Time($collection, $componentOption);

            case self::COM_YMD:
                $collection = array_try_get($componentOption, self::FIELD_DATA, [], true);
                return new Ymd($collection, $componentOption);

            case self::COM_PCA:
                $collection = array_try_get($componentOption,self::FIELD_DATA,[],true);
                return new Pca($collection,$componentOption);

            case self::COM_TIME_RANGE:
                $collection = array_try_get($componentOption, self::FIELD_DATA, [], true);
                return new TimeRange($collection, $componentOption);

            case self::COM_CHECK_GROUP:
                $collection = array_try_get($componentOption, self::FIELD_DATA, [], true);
                return new CheckGroup($collection, $componentOption);
            ...
            case self::COM_IMAGE_UPLOAD:
                return new ImageUpload($componentOption);

            case self::COM_RICH_TEXT:
                return new RichText($componentOption);
