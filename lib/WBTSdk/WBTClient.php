...
class WBTClient
{
    ...
    {
        $redirectUrl = $this->getAuthorizeURL($url, $state);
    ...
    function getAuthorizeURL($url, $state = null)
    {
        ...
        $signature = md5($resultURL . $this->_secret);
    ...
    protected function _api($api, array $params = null, $isRetry = false)
    {
        ...
        try
        {
            $response = $this->_send($api, null, null, $params);
        }
    ...
    protected function _send($endpoint, array $extraHeaders = null, array $queryParams = null, array $postParam = null)
    {
        $url = $this->_apiEntry . $endpoint;
        ...
        {
            curl_close($ch);
            throw new WBTApiException(curl_error($ch), curl_errno($ch));
        }
