...
class MallController extends WxUserControllerBase
{
...
*/
        $con = [Product::MP_USER_ID => $mpUserID,Product::STORE_ID => $storeID,Category::IS_DELETE => 0,Category::IS_ON_SHELF => 1];
        ...
        {
            $conCategory[] = $value[Category::CATEGORY_ID];
        }
        ...
        //    $categoryID = $dataDefault[Category::CATEGORY_ID];
         //   $condition = [Product::MP_USER_ID => $mpUserID,Product::STORE_ID => $storeID,Product::CATEGORY_ID => $categoryID];
            $condition = [Product::MP_USER_ID => $mpUserID,Product::STORE_ID => $storeID,Product::CATEGORY_ID => $conCategory];
            $categoryName = '首页';
        }
                ...
                foreach($dataCategory as $k => $v)
                {
                    ...
                    if($v[Category::CATEGORY_ID] == $categoryID)
                    {
                     ...
                     //   unset($dataCategory[$k]);
                        $dataCategory[$k][Category::TITLE] = '首页';
                        break;
                    }
                }
                $condition = [Product::MP_USER_ID => $mpUserID,Product::STORE_ID => $storeID,Product::CATEGORY_ID => $categoryID];
...
//  拼接产品表中   is_delete为 0 ,is_on_shelf == 1 的产品，也就是没有被删除的产品
        ...
        $cart = new Cart([Cart::MP_USER_ID => $mpUserID,Cart::WX_USER_ID => $wxUserID,Cart::STORE_ID => $storeID]);
        if(!$cart->isEmpty())
        {
...
//读取评论
        ...
        $condition = [ProductComment::MP_USER_ID => $mpUserID,ProductComment::PRODUCT_ID => $productID];
        ...
        if ($from)
        {
            $this->changeView('WBT/WxUser/Store.detail.html');
        }
    ...
    public function shoppingAction()
    {
        ...
        $this->_view->set("mp_user_id",$mpUserID);
        $this->_view->set("wx_user_id",$wxUserID);
        log_debug("[$mpUserID][$wxUserID][$storeID]");
        $cart = new Cart([Cart::MP_USER_ID => $mpUserID,Cart::WX_USER_ID => $wxUserID,Cart::STORE_ID => $storeID]);
        if(!$cart->isEmpty())
        {
            $cartID = $cart -> getCartID();
