...
class ZhongaoController extends WxUserControllerBase
{
    ...
    public function userAction()
    {
        ...
        $houseMember = new HouseMember([HouseMember::WX_USER_ID => $this->_wxUserID,HouseMember::MEMBER_TYPE => HouseMemberType::OWNER]);
        if ($houseMember->isEmpty())
        {
            exit('对不起，你还不是本社区的业主或住户，不能执行该操作');
        }
// houseId 为空
        $condition = [HouseMember::WX_USER_ID => $this->_wxUserID,HouseMember::MEMBER_TYPE => HouseMemberType::OWNER];
        ...
        $condition = [HouseMember::HOUSE_ADDRESS => $address,HouseMember::ADD_BY   => $this->_wxUserID];
