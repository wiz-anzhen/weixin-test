...
class CsChatRecordCwbController extends CwbControllerBase
{
//长微博回复
    public function answerCwbAction()
    {
...
//长微博所对应的所有客户
    public function answerCwbTableAction()
    {
        $mpUserID = $this->_request->getQueryParam("mp_user_id");
        ...
        if(strpos($_SERVER["HTTP_USER_AGENT"], "MicroMessenger"))
        {
            ...
            $cs = new CustomerSpecialist([CustomerSpecialist::CUSTOMER_SPECIALIST_ID => $csID]);
            $csWxUserID = $cs->getWxUserID();
            if($csWxUserID != $wxUserID)
            {
        ...
        $condition = [$con, 'mp_user_id' => $mpUserID];
        $wxUserIds =  CsChatRecord::fetchColumn([CsChatRecord::WX_USER_ID],$condition);
        $arrayWxUserId = [];
        ...
        //找出同一个微信id下所有cs_chat_record_id记录
        $csChatRecordInfo = [];
        foreach($csChatRecord as $value)
        {
        ...
        $shownColumns = [
            'name' => [
                Table::COLUMN_TITLE => 'name',
                Table::COLUMN_FUNCTION => function (array $row)use($wxUserInfo,$mpUserID,$csID)
                    {
                        ...
                        $url = sprintf(" <a href='%s/wx_user/cs_chat_record_cwb/answer_cwb?type=1&wx_user_id=%s&mp_user_id=%s&cs_id=%s#bottom-body'>$retValue</a>",$host,$wxUserID,$mpUserID,$csID);
