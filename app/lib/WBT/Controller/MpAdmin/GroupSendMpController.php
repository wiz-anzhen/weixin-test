...
class GroupSendMpController extends CommunityControllerBase
{
    protected function _init()
    {
        $this->_moduleName = "send_by_group_mp";
        parent::_init();
    }

    //公众号级别
    public function sendMpAction()
    {
        ...
        $condition     = [ GroupSend::MP_USER_ID => $mpUserId,GroupSend::SEND_TYPE => "mp"];
        ...
        $groupSendTime = GroupSend::fetchColumn([GroupSend::GROUP_SEND_TIME],[GroupSend::COMMUNITY_ID => $communityId,GroupSend::GROUP_SEND_RANGE => GroupSendRangeType::SEND_TO_MP_USER]);
        if($mpUserType)
        {
            $count = [];
        ...
        $shownColumns = [
                ...
                Table::COLUMN_FUNCTION => function (array $row)
                    {
                        ...
                        {
                            $url = sprintf("/mp_admin/group_send_mp/content?mp_user_id=%s&group_send_id=%s&community_id=%s", $row[GroupSend::MP_USER_ID], $row[GroupSend::GROUP_SEND_ID], $row[GroupSend::COMMUNITY_ID]);
            ...
            Table::COLUMN_OPERATIONS => [
                ...
                Table::COLUMN_FUNCTION =>function(array $row)use($remainCount){
                        ...
                        $update = new Link('修改', "javascript:bluefinBH.ajaxDialog('/mp_admin/group_send_dialog/update?group_send_id={$groupSendId}&community_id={$community_id}&from=mp');");
                        $delete = new Link('删除', "javascript:bluefinBH.confirm('警告：删除后，该群发信息将丢失，且无法恢复。<br/><br/>确定要删除吗？', function() { javascript:wbtAPI.call('../fcrm/group_send/delete?group_send_id={$groupSendId}&community_id={$community_id}&from=mp', null, function(){bluefinBH.showInfo('删除成功', function() { location.reload(); }); }); })");                           $send = new Link('发布', "javascript:bluefinBH.confirm('确定要发布消息吗？', function() { javascript:wbtAPI.call('../fcrm/group_send/send?group_send_id={$groupSendId}&mp_user_id={$mpUserID}&community_id={$community_id}&from=mp', null, function(){bluefinBH.showInfo('发布成功', function() { location.reload(); }); }); })");
                        $copy = new Link(' 复制', "javascript:bluefinBH.confirm('确定要复制此消息吗？', function() { javascript:wbtAPI.call('../fcrm/group_send/copy?group_send_id={$groupSendId}&mp_user_id={$mpUserID}&community_id={$community_id}&from=mp', null, function(){bluefinBH.showInfo('复制成功', function() { location.reload(); }); }); })");
                        $preview =  new Link('预览', "javascript:bluefinBH.ajaxDialog('/mp_admin/group_send_dialog/preview?group_send_id={$groupSendId}&community_id={$community_id}&from=mp');");
