...
class UserNotifyMpController extends CommunityControllerBase
{
    protected function _init()
    {
        $this->_moduleName = "user_notify_mp";
        parent::_init();
    }
    //公众账号级别发送
    public function subscribeAction()
    {
        ...
        $condition     = [ UserNotify::MP_USER_ID => $mpUserId,UserNotify::SEND_TYPE => "mp"];
                ...
                Table::COLUMN_FUNCTION => function(array $row)
                    {
                        ...
                        $update = new Link('修改', "javascript:bluefinBH.ajaxDialog('/mp_admin/user_notify_dialog/update?user_notify_id={$userNotifyId}&community_id={$community_id}&from=mp');");
                        ...
                        $send = new Link('发布', "javascript:bluefinBH.confirm('确定要发布消息吗？', function() { javascript:wbtAPI.call('../fcrm/user_notify/send?user_notify_id={$userNotifyId}&mp_user_id={$mpUserID}&community_id={$community_id}&from=mp', null, function(){bluefinBH.showInfo('发布成功', function() { location.reload(); }); }); })");
                        $copy = new Link(' 复制', "javascript:bluefinBH.confirm('确定要复制此消息吗？', function() { javascript:wbtAPI.call('../fcrm/user_notify/copy?user_notify_id={$userNotifyId}&mp_user_id={$mpUserID}&community_id={$community_id}&from=mp', null, function(){bluefinBH.showInfo('复制成功', function() { location.reload(); }); }); })");
                        ...
                        if($row[UserNotify::SEND_STATUS] == UserNotifySendStatus::SEND_NO)
                        {
                            $ret = $update."<br>".$delete."<br>".$preview."<br>".$send;
                        }
                        elseif($row[UserNotify::SEND_STATUS] == UserNotifySendStatus::SEND_WAIT)
                        {
                            $ret = $update."<br>".$delete;
                        }
                        else
                        {
                            $ret = $copy."<br>".$delete;
