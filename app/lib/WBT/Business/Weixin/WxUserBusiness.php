...
// 普通微信用户相关的一些操作
class WxUserBusiness extends BaseBusiness
{
    ...
    public static function initWxUserForSession(MpUser &$mpUser, $wxUserID)
    {
        ...
        else
        {
            ...
            $wxUser->setLastAccess(date('Y-m-d H:i;s',time()))->update();
        }
        // 更新用户头像及昵称
        $headPic = $wxUser->getHeadPic();
        log_debug("====================".$headPic);
        $nickname = $wxUser->getNick();
        $city= $wxUser->getWCity();
        $province= $wxUser->getWProvince();
        $gender= $wxUser->getGender();
        if(empty($headPic) || empty($nickname)|| empty($province)|| empty($city)|| empty($gender))
        {
            $headPic = WxApiBusiness::getHeadImgUrl($mpUserID,$wxUserID,$nickname,$city,$province,$gender);
            if(!empty($headPic))
            {
            ...
            if(!empty($province))
            {
                $wxUser->setWProvince($province)->update();
            }
            if(!empty($gender))
            {
                $wxUser->setGender($gender)->update();
            }
        }
    ...
    //通过经纬度获取地址
    public static function getAddressByPrecision($lng,$lat)
    {
        $lng = urlencode($lng);
        $lat = urlencode($lat);
        $json=file_get_contents("http://api.map.baidu.com/geocoder/v2/?ak=E4805d16520de693a3fe707cdc962045&callback=renderReverse&location=".$lat.",".
    ...
    public static function getWxUserList( array $condition, array &$paging = NULL, $ranking,  array $outputColumns = NULL )
    {
        return WxUser::fetchRowsWithCount( [ '*' ], $condition, NULL, $ranking, $paging,$outputColumns );
    }
    ...
    public static function checkAndBindMemberPhone($communityId,$mpUserID, $wxUserID, $phone, $code)
    {
        ...
        $wxUser->setPhone($phone)
            ...
            ->setCurrentCommunityID($communityId)->setBirth($houseMember->getBirthday())->setAddress($houseMember->getHouseAddress())->setIdentifyTime(time())->setIsQuit(0)
            ->update();
    ...
    //注册+认证
    public static function checkAndBindMemberPhoneRegisterAndIdentify($communityId,$mpUserID, $wxUserID, $phone, $code)
    {
        $wxUser = new WxUser([WxUser::WX_USER_ID => $wxUserID]);
        if($wxUser->isEmpty())
        {
        ...
        $community = new Community([Community::NAME => $mpUserName,Community::MP_USER_ID=>$mpUserID]);
        //注册
        $wxUser->setPhone($phone)->setRegisterTime(time())->setCurrentCommunityID($community->getCommunityID())->update();
        //认证
        $hm = new HouseMember();
        ...
        $hm_data = [HouseMember::HOUSE_NO=>$wxUser->getVipNo(),

            HouseMember::MP_USER_ID=>$mpUserID,
            HouseMember::COMMUNITY_ID=>$community->getCommunityID(),
            HouseMember::NAME => $nick,
            HouseMember::WX_USER_ID => $wxUserID,
            HouseMember::PHONE1 => $phone,
            HouseMember::MEMBER_TYPE=>"member",
            HouseMember::VERIFY_TIME=>date('Y-m-d H:i:s',time())
        ];
    ...
    //新的认证流程
    public static function checkNewPhoneExist($wxUserId, $mpUserId, $phone,$type)
    {
        $expr = sprintf("`phone1` = %s or `phone2` = %s or `phone3`=%s", $phone, $phone, $phone);
        $dbCondition = new \Bluefin\Data\DbCondition($expr);

        $condition = [$dbCondition,HouseMember::MP_USER_ID=>$mpUserId];

        $communityID_array = HouseMember::fetchRows([HouseMember::COMMUNITY_ID],$condition);
        if(empty($communityID_array))
        {
            return ['errno' => 2];
        }
        foreach($communityID_array as $communityID)
        {
            $community = new Community([Community::COMMUNITY_ID=>$communityID[Community::COMMUNITY_ID],
                                        Community::VALID=>1,
                                        Community::IS_VIRTUAL=>0,
                                        Community::IS_APP=>0]);
            if(!$community->isEmpty())
            {
                $community_code = ['communityID' => $communityID[Community::COMMUNITY_ID] ,'communityName' => $community->getName()];
                $communityArr[] = $community_code;
            }
        ...
        if(!$verifyCodeID[MpUserConfigType::VERIFY_CODE_ID])
        {
            ...
            return ['errno' => 0,'community'=> $communityArr];
