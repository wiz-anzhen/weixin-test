...
/* 点菜相关业务 */
class CommunityAdminBusiness extends BaseBusiness
{

    //列表的显示
    public static function getCommunityAdminList(array $condition, array &$paging = null, $ranking, array $outputColumns = null)
    {
        return CommunityAdmin::fetchRowsWithCount( [ '*' ], $condition, null, $ranking, $paging, $outputColumns );
    }
    //数据的录入
    public static function insert($data,$password,$userName)
    {
        ...
        {/*
            ...
            $obj->setPassword("88888888")->setUsername($userName)->setStatus("activated")->insert();
        }
        else
        {/*
            if(!empty($password))
    ...
    public static function update( $id,$data )
    {
        ...
        if ($obj->isEmpty()) {
            log_debug( "Could not find CommunityAdmin($id)" );
