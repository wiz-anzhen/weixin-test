...
CREATE TABLE IF NOT EXISTS `order_change_log` (
    `order_change_log_id` INT(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
    ...
    `status_before` CHAR(32) NOT NULL DEFAULT 'default_status' COMMENT '变更前状态',
    `status_after` CHAR(32) NOT NULL DEFAULT 'default_status' COMMENT '变更后状态',
    PRIMARY KEY (`order_change_log_id`),
    KEY `ak_order_change_log_order_id` (`order_id`),
    KEY `ak_order_change_log_change_time` (`change_time`)
