...
class AppmallService extends Service{
    public function shopping()
    {
        ...
        $cartNew = new Cart([Cart::MP_USER_ID => $mpUserID,Cart::WX_USER_ID => $phone,Cart::STORE_ID => $storeID]);
        if($cartNew->isEmpty())
        {
            $cartNew -> setMpUserID($mpUserID)->setStoreID($storeID)->setWxUserID($phone)->insert();
        }
        //获取购物车ID
...
log_debug("------service----------[$productID][$cartID]-----------------");
    ...
    public function orderDetail()
    {
        ...
        log_debug("-------------[$mpUserID][$phone][$communityID][$nick][$tel][$address][$status][$createTime][$orderID][$totalNum][$totalMoney]------------------");

        //从house_membe表中获取客服信息
        $currentCsGroupID = $currentCsID = '';
        $houseMember = new HouseMember([HouseMember::MP_USER_ID => $mpUserID,HouseMember::PHONE1 => $phone]);
