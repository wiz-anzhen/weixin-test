...
// 普通用户点菜相关 api
class AppOrderService extends Service
{
    ...
    public function checkout()
    {
        ...
        $phone = $this->_app->request()->getPostParam( 'phone' );
        ...
        if(!$appUser->isEmpty())
        {
            $appUser->setNick($name)->setAddress($address)->update();
        }
        ...
        $hm = new HouseMember([HouseMember::PHONE1 => $phone]);
        $csID = $hm->getCurrentCsID();
        $csGroupID = $hm->getCurrentCsGroupID();
        $status = "paid_to_verify";
        $order = new Order();
    ...
    {
        $fields = [ 'phone', 'store_id', 'product_id', 'action', 'mp_user_id'];
        $data   = $this->_app->request()->getArray( $fields );
        ...
        if (empty($data['phone'])) {
            return ['errno' => 1, 'error' => '非法操作，缺少PHONE'];
        } else {
            //$wxUser = new WxUser([WxUser::WX_USER_ID => $data['wx_user_id']]);
            $appUser = new AppUser([AppUser::PHONE=>$data['phone']]);
            if ($appUser->isEmpty()) {
                return ['errno' => 0, 'error' => '找不到用户，视为演示操作'];
            }
        }
        ...
        //$newOrderId = OrderBusiness::getNewOrder($data['wx_user_id'], $data['store_id']);
        //return OrderBusiness::changeOrder($newOrderId, $data['product_id'], $data['action']);
        $cartId = OrderBusiness::getCartId($data['phone'], $data['store_id']);
    ...
    public function addComment()
    {
        ...
        unset($arrP[0]);
        $count = count($arrS);
        log_debug("-----------------------[$mpUserID][$phone][$communityID][$orderID][$commentTime][$stars][$textareas][$pids][$maxPCID]--------------------");
        for($i=1;$i<=$count;$i++)
        {
            log_debug("------------------------------------ok0------------------------------------------");
            $star = $arrS[$i];
            ...
            $productComment = new ProductComment();//此处未加小区id
                log_debug("------------------------------------ok8------------------------------------------");
                $maxPCID = $maxPCID + 1;
                $productComment -> setMpUserID($mpUserID)->setCommunityID($communityID)->setWxUserID($phone)->setProductID($pid)->setCommentLevel($star)->setComment($textarea)->setProductTitle($title)->setOrderFinishTime($time)->setCommentTime($commentTime)->setProductCommentID($maxPCID)->setOrderID($orderID)->setNick($appUser->getNick())->setHeadPic($appUser->getHeadPic())->insert();
                log_debug("------------------------------------ok9------------------------------------------");
        }
        return $res;
    }
